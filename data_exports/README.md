...
### Notion
Create a Notion account, then go into Settings > Import, and import `notion_mcp_eval_export.zip`. Then go to [https://www.notion.so/profile/integrations](https://www.notion.so/profile/integrations) and add a new integration (type is Internal) and get the `Internal Integration Secret` and save it as `NOTION_TOKEN` in `.env`
