...
#   --api-key="GEMINI_API_KEY"  # optional, recommend putting in .env \
...
class EvaluatorConfig:
    ...
    num_tasks: Optional[int] = None  # Limit number of tasks to process
...
def initialize_gemini(config: EvaluatorConfig):
    """Initialize Gemini with config"""
    api_key = config.api_key or os.getenv("GEMINI_API_KEY")
    if not api_key:
        raise ValueError("Gemini API key not found. Set GEMINI_API_KEY env var or pass in config.")
...
class AsyncGeminiClient:
    """Manages async Gemini requests with rate limiting"""
    ...
    async def generate_structured_content(self, prompt: str, response_schema: Dict, temperature: float = 0.0) -> Dict:
        """Generate structured content using Gemini with retry logic."""
...
class CoverageEvaluator:
    ...
    def __init__(self, client: AsyncGeminiClient, config: EvaluatorConfig):
        self.client = client
        self.config = config
        self.logger = logging.getLogger(__name__)
...
class LLMTrajectoryComparator:
    ...
    def __init__(self, config: EvaluatorConfig):
        self.config = config
        self.client = AsyncGeminiClient(config)
        self.logger = logging.getLogger(__name__)
...
async def main(args):
    ...
    try:
        ...
        client = AsyncGeminiClient(config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-file", type=str, required=True, help="Path to the completion results CSV file containing both ground truth and model outputs (e.g., completion_results/sample_4o_results.csv).")
    parser.add_argument("--model-name", type=str, required=True, help="Short name for the model being evaluated (e.g., 'gpt4o'). Used for naming output files.")
    parser.add_argument("--evaluator-model", type=str, default="gemini-2.5-pro", help="Name of the Gemini model to use for evaluation scoring.")
    parser.add_argument("--api-key", type=str, default=None, help="Gemini API key. If not provided, it will check for the GEMINI_API_KEY environment variable.")
    parser.add_argument("--output-dir", type=str, default="evaluation_results", help="Directory to save all output files.")
    parser.add_argument("--concurrency", type=int, default=5, help="Number of concurrent requests to the Gemini API.")
    parser.add_argument("--num-tasks", type=int, default=None, help="Limit evaluation to first N tasks (useful for testing). If not specified, processes all tasks.")
