                                    ...
                                    for tool_call in tool_calls:
                                        if isinstance(tool_call, dict) and 'function' in tool_call:
                                            ...
                                            if isinstance(function_info, dict) and 'name' in function_info:
                                                ...
                                                mapped_tool_name = TOOL_NAME_MAPPINGS.get(tool_name, tool_name)
                                                if MCP_SERVER_NAME_ONLY:
                                                    mapped_tool_name = mapped_tool_name.split("_")[0]
                                                task_tools[task_id].add(mapped_tool_name)
                    
                    except json.JSONDecodeError as e:
                        print(f"Warning: Could not parse JSON in TRAJECTORY for task {task_id}: {e}", file=sys.stderr)
                        continue
