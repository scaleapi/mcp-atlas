...
class AsyncMCPTrajectoryGenerator:
    ...
    async def process_single_task(self, row_data: Dict[str, Any], output_file: str, 
        ...
        num_attempts = 0  # Initialize retry count
        ...
        try:
            ...
            logging.info(f"Sleeping for {random_sleep} seconds")
...
async def main():
    ...
    if filter_enabled:
        ...
        if not enabled_servers:
            ...
            # For HuggingFace datasets, we need to save to a temp CSV first
            if csv_filename is None:
                import tempfile
                with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
                    df.to_csv(f.name, index=False)
                    csv_filename = f.name
                logging.info(f"Saved HuggingFace dataset to temp file for filtering: {csv_filename}")
