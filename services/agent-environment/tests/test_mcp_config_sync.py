...
def test_package_sync():
    ...
    if missing_npx:
        ...
        error_msg = ["❌ Install script does not match template (template is source of truth):", ""]
    ...
    print(f"✅ All {len(config_regular_npx)} NPX packages from template are in install script!")
    print(f"✅ All {len(config_regular_uvx)} UVX packages from template are in install script!")
